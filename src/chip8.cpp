...
void print_hex(unsigned short c) {
    cout << hex << "0x" << ((c < 0x10) ? "0" : "") << ((c < 0x100) ? "0" : "")
         << ((c < 0x1000) ? "0" : "") << (static_cast<unsigned int>(c) & 0xFFFF) << dec << endl;
}
...
void Chip8::emulateCycle() {
    if (op_error)
        return;
    // Fetch opcode
    opcode = memory[pc] << 8 | memory[pc + 1]; // Uses bit-shifting and bitwise OR to combine the
                                               // two byte opcode into one piece of data
    ...
    case 0x8000:
        ...
        case 0x0005: // 8XY5: VY is subtracted from VX. VF is set to 0 when there's a borrow, else 1
            ...
            pc += 2;
            break;
        case 0x0006: // 8XY6: Stores the least sig. bit of VX in VF and then shifts VX to the right
                     // by 1
            V[0xF] = V[X] & 0x000F;
            V[X] >>= 1;
            pc += 2;
        ...
        case 0x0007: // 8XY7: Sets VX to VY minus VX. VF is set to 0 when there's a borrow, else 1
            ...
            pc += 2;
            break;
        case 0x000E: // 8XYE: Stores the most sig. bit of VX in VF and then shifts VX to the left by
                     // 1
    ...
    case 0xB000: // BNNN: Jumps to the address NNN plus V0
        pc = NNN + V[0];
        break;
    case 0xC000: // CXNN: Sets VX to the result of a bitwise and operation on a random number (0 to
                 // 255) and NN
        V[X] = (rand() % 255) & NN;
        pc += 2;
        break;
    ...
    case 0xF000:
        ...
        case 0x0029:      // FX29: Sets I to the location of the sprite for the character in VX
            ...
            pc += 2;
            break;
        case 0x0033: // FX33: Stores the binary-coded decimal representation of VX. See reference
                     // table for specs.
